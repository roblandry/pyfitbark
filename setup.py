...
required_test = [line for line in open('requirements/test.txt').read().split("\n") if
                 not line.startswith("-r") and line != '']

fbinit = open('pyfitbark/__init__.py').read()
...
setup(
    name='pyfitbark',
    version=version,
    description='Custom FitBark API Wrapper.',
    long_description=open('README.rst').read(),
    author=author,
    author_email=author_email,
    url='https://github.com/roblandry/pyfitbark',
    packages=['pyfitbark'],
    package_data={'': ['LICENSE']},
    include_package_data=True,
    install_requires=["setuptools"] + required,
    license='Apache 2.0',
    test_suite='pyfitbark_tests.all_tests',
